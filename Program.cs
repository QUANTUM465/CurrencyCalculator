...
namespace ConsoleСurrencyCalculator
{
    ...
    internal class Program
    {
        ...
        /// <returns>
        /// Return object of Class currency
        /// </returns>
        public static Currency FindCurrency(int codeA, int codeB)
        {
            ...
            try
            {
                foreach (var item in currencies)
                {
                    ...
                    {
                        // TODO:: Треба переробити логіку
                        if (item.CurrencyCodeA == codeA && item.CurrencyCodeB == 980)
                        {
        ...
        //UNDONE:: Server throw exception if number of request is too big
        ...
        public static List<Currency> GetCurrency()
        {
            ...
            //var url = "https://api.monobank.ua/bank/currency"; // Адреса сайту 
            //var client = new HttpClient();
            //var response = client.GetAsync(url).Result; // Витягуємо відповідь на запит за вказаною адресою
            //var json = response.Content.ReadAsStringAsync().Result; // Читаємо як стрічку відаовідь на запит
            var path = "C:\\Users\\taras\\Desktop\\currency.json";
            var json = File.ReadAllText(path);

            var currencies = JsonConvert.DeserializeObject<List<Currency>>(json); // Парсимо стрічку і додаємо об'кти класу в List
            return currencies;
        }
    }
}
